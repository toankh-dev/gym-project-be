...
module.exports = {
    async up(queryInterface, Sequelize) {
        ...
        const [memberRows] = await queryInterface.sequelize.query('SELECT id FROM members WHERE id IN (1, 2, 3) OR id BETWEEN 1000 AND 1099');
        const memberIds = memberRows.map((r) => r.id);
        const [packageRows] = await queryInterface.sequelize.query('SELECT id, price, duration_months FROM membership_packages WHERE status = "ACTIVE"');
        ...
        // Payments
        const [allSubs] = await queryInterface.sequelize.query('SELECT id, member_id, actual_price, created_at FROM member_subscriptions WHERE actual_price IS NOT NULL');
        const payments = [];
...
//# sourceMappingURL=017-member-extras.js.map