...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    const [memberRows] = await queryInterface.sequelize.query(
      'SELECT id, assigned_trainer_id FROM members WHERE assigned_trainer_id IN (1, 2)'
    );
    ...
    const [futureSchedules] = await queryInterface.sequelize.query(
      "SELECT id, trainer_id, max_capacity, current_enrollment, start_date FROM training_schedules WHERE start_date >= CURDATE() AND status='SCHEDULED'"
    );
    ...
    const [existingPairs] = await queryInterface.sequelize.query(
      'SELECT schedule_id, member_id FROM schedule_members'
    );
    ...
    const [attendedRows] = await queryInterface.sequelize.query(
      "SELECT id FROM schedule_members WHERE attendance_status='ATTENDED' AND id >= 1000 AND id < 5000 AND notes IS NULL ORDER BY id LIMIT 80"
    );

    for (const row of attendedRows) {
      const note = pick(ATTENDANCE_NOTES).replace(/'/g, "''");
      await queryInterface.sequelize.query(
        `UPDATE schedule_members SET notes = '${note}' WHERE id = ${row.id}`
      );
    }
  },
  ...
  async down(queryInterface, Sequelize) {
    ...
    await queryInterface.sequelize.query(
      "UPDATE schedule_members SET notes = NULL WHERE attendance_status='ATTENDED' AND id >= 1000 AND id < 5000"
    );
  },
};
